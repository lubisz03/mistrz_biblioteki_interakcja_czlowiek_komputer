...
export default function Quiz() {
  ...
  const [finalScore, setFinalScore] = useState<{ player1: number; player2: number; winner: number | null } | null>(
    null
  );
  ...
  const currentQuestionIndexRef = useRef<number>(0); // Ref do aktualnego indeksu dla callbacków
  ...
  // Funkcja pomocnicza do aktualizacji currentQuestionIndex i synchronizacji ref
  const updateCurrentQuestionIndex = (newIndex: number | ((prev: number) => number)) => {
    setCurrentQuestionIndex((prev) => {
      const resolvedIndex = typeof newIndex === 'function' ? newIndex(prev) : newIndex;
      currentQuestionIndexRef.current = resolvedIndex;
  ...
  useEffect(() => {
    ...
    if (!matchId) {
      console.log('[Quiz] No matchId -> redirect /');
    ...
    if (matchEnded || match?.status === 'finished' || resultsFetchedRef.current || finalScore) {
      console.log('[Quiz] Match finished or results fetched, skip WS', {
        matchEnded,
        matchStatus: match?.status,
        resultsFetched: resultsFetchedRef.current,
        finalScore: !!finalScore,
    ...
    const handleQuestion = (questionData: any) => {
      if (!isMounted) return;
      console.log('[Quiz] handleQuestion', questionData);
      ...
      if (question && question.question_text) {
        // Sprawdź czy to nowe pytanie (inny ID niż aktualne) - używaj REF aby uniknąć stale closure!
        const currentQ = currentQuestionRef.current;
        const isNewQuestion = !currentQ || currentQ.id !== question.id;
        ...
        if (isNewQuestion) {
          ...
          currentQuestionRef.current = questionObj; // Zaktualizuj ref PRZED innymi stanami
          setStoreQuestion(question);
          setSelectedAnswer(null);
          // NIE resetuj showResult tutaj! handleMatchResult ma timeout który to robi.
          // Jeśli zresetujemy tutaj, a match:question przyjdzie zaraz po match:result,
          // użytkownik nie zobaczy wyniku (React batch'uje zmiany stanu).
    ...
    const handleMatchStart = (data: any) => {
      console.log('[Quiz] match:start', data);
      ...
      // Sprawdź czy to nowe pytanie przed obsługą - używaj REF aby uniknąć stale closure
    ...
    const handleMatchQuestion = (data: any) => {
      console.log('[Quiz] match:question', data);
      ...
      if (questionData?.current_question_index !== undefined) {
        ...
        updateCurrentQuestionIndex((prev) => {
    ...
    const handleMatchResult = (data: { data: MatchResult }) => {
      if (!isMounted) return;
      console.log('[Quiz] match:result', data);
      ...
      // WAŻNE: Synchronizuj selectedAnswer z your_answer z backendu
      // To zapewnia spójność nawet gdy useEffect się reruruje
      // Używamy funkcji callback w setSelectedAnswer aby uniknąć stale closure
      if (result.your_answer) {
        setSelectedAnswer((prevSelected) => {
          if (!prevSelected) {
            logger.debug('Quiz: Syncing selectedAnswer from backend:', result.your_answer);
            return result.your_answer;
          }
          return prevSelected;
        });
      }
    ...
    const handleMatchEnd = (data: { data: any }) => {
      if (!isMounted) return;
      console.log('[Quiz] match:end', data);
    ...
    const handleOpponentAnswered = () => {
      console.log('[Quiz] match:opponent_answered');
    ...
    const handleTimerSync = (data: { time_left: number; question_index: number }) => {
      ...
      if (data.question_index === currentIdx) {
        ...
        if (data.question_index > currentIdx) {
          ...
          updateCurrentQuestionIndex(data.question_index);
          setTimeLeft(data.time_left);
        } else {
    ...
    const handleOpponentDisconnect = (data: { message?: string }) => {
      if (!isMounted) return;
      console.log('[Quiz] match:opponent_disconnect', data);
    ...
    const handleMatchTimeout = (data: { message?: string; question_index?: number }) => {
      if (!isMounted) return;
      console.log('[Quiz] match:timeout', data);
    ...
    const handleMatchAlreadyEnded = (data: { message?: string }) => {
      if (!isMounted) return;
      console.log('[Quiz] match:already_ended', data);
    ...
    const fetchTokenAndConnect = async () => {
      try {
        console.log('[Quiz] fetchTokenAndConnect start');
        ...
        if (match?.status === 'finished' || matchEnded || resultsFetchedRef.current) {
          console.log('[Quiz] skip connect, match finished');
          logger.debug('Quiz: Match already finished, skipping socket connection', {
            matchStatus: match?.status,
            matchEnded,
            resultsFetched: resultsFetchedRef.current,
        ...
        if (existingSocket && existingSocket.matchId === parseInt(matchId)) {
          ...
          if (ws && (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING)) {
            console.log('[Quiz] Reusing existing socket', ws.readyState);
            logger.debug('Quiz: Reusing existing socket connection');
            socketInstance = existingSocket;
            socketConnectedRef.current = true;

            // WAŻNE: Wyczyść stare listenery przed rejestracją nowych
            // To zapobiega duplikatom gdy useEffect się reruruje
            if (cleanupRef.current) {
              cleanupRef.current();
            }
            ...
            if (ws.readyState === WebSocket.OPEN) {
              console.log('[Quiz] Socket open -> send match:ready');
        ...
        console.log('[Quiz] tokenResponse', tokenResponse.data);
        const token = tokenResponse.data?.token;

        if (!token) {
          console.log('[Quiz] No token from /auth/token/websocket');
        ...
        console.log('[Quiz] connectSocket created', { matchId, hasSocket: !!socketInstance });
        socketConnectedRef.current = true;

        // Wyczyść stare listenery przed rejestracją nowych
        if (cleanupRef.current) {
          cleanupRef.current();
        }
        ...
        if (ws && ws.readyState === WebSocket.OPEN) {
          console.log('[Quiz] Socket already open after connectSocket -> send match:ready');
          logger.debug('Quiz: Socket already open, sending match:ready');
          socketInstance.send('match:ready');
        } else {
          console.log('[Quiz] Socket not open, calling connect()');
          socketInstance
            .connect()
            .then(() => {
              console.log('[Quiz] Socket connected -> send match:ready');
            ...
            .catch((error: unknown) => {
              console.log('[Quiz] Socket connection error', error);
        ...
        console.log('[Quiz] fetchTokenAndConnect error', error);
    ...
    if (!resultsFetchedRef.current && !matchEnded && !isMatchFinished && !finalScore) {
      console.log('[Quiz] will connect to WS');
      fetchTokenAndConnect();
    } else if (matchEnded || isMatchFinished || finalScore) {
      console.log('[Quiz] match ended -> ensure socket closed');
    ...
    const questionTimeout = setTimeout(() => {
      if (
        isMounted &&
        !questionRequestedRef.current &&
        !matchEnded &&
        match?.status !== 'finished' &&
        !resultsFetchedRef.current
      ) {
        // Sprawdź czy mamy pytanie używając refa
        if (!currentQuestionRef.current) {
          console.log('[Quiz] No question after 2s -> send match:ready');
    ...
    return () => {
      console.log('[Quiz] cleanup useEffect');
    ...
    // UWAGA: Używamy match?.status zamiast match aby uniknąć nadmiernych re-runów
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [matchId, navigate, setStoreQuestion, addQuestionResult, match?.status, currentUser?.id, matchEnded, finalScore]);

  const handleAnswer = (answer: string) => {
    if (selectedAnswer || showResult) {
      logger.debug('Quiz: handleAnswer called but answer already selected or showing result', {
        selectedAnswer,
        showResult,
      });
      return;
  ...
  const getAnswerStatus = (option: string) => {
    ...
    // WAŻNE: Używamy your_answer z backendu jako źródło prawdy, nie selectedAnswer!
    // selectedAnswer może być niezsynchronizowane po rerun useEffect
    const userAnswer = lastResult.your_answer || selectedAnswer;
        ...
        <div className="min-h-[60vh] flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-semibold text-gray-800 animate-pulse">Oczekiwanie na pytanie...</h2>
              <p className="text-gray-600">Przygotowujemy quiz dla Ciebie</p>
            </div>
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <ProgressBar current={currentQuestionIndex + 1} total={match?.total_questions || 10} />
        </div>
        ...
        {match && (
          <div className="mb-6">
            <OpponentStatus
              opponent={currentUser?.id === match.player1?.id ? match.player2 || undefined : match.player1 || undefined}
        ...
        <QuestionTransition key={currentQuestion.id || questionResults.length}>
          <div className="bg-white rounded-lg shadow-lg p-8 mb-6 animate-scale-in">
            <h2 className="text-2xl font-bold text-gray-800 mb-6 animate-fade-in">{currentQuestion.question_text}</h2>
        ...
        {showResult &&
          ...
          (() => {
            ...
            // ZABEZPIECZENIE: Pokaż wynik TYLKO jeśli dotyczy aktualnego pytania
            // Jeśli currentQuestion się zmienił (bo przyszło match:question przed timeout),
            // nie pokazuj wyniku starego pytania na ekranie z nowym pytaniem!
            if (lastResult.question?.id !== currentQuestion?.id) {
              logger.debug('Quiz: Result question ID mismatch, not showing result overlay', {
                resultQuestionId: lastResult.question?.id,
                currentQuestionId: currentQuestion?.id,
              });
              return null;
            }
            
            // ZAWSZE używaj your_correct z backendu - to jest jedyne źródło prawdy!
            // selectedAnswer może być niezsynchronizowane po rerun useEffect
            const userAnsweredCorrectly = lastResult.your_correct;

            return (
              <div
                className={`rounded-lg p-4 mb-6 border-2 animate-bounce-in ${
                  userAnsweredCorrectly ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                }`}
              >
                <p
                  className={`text-lg font-semibold flex items-center gap-2 ${
                    userAnsweredCorrectly ? 'text-green-800' : 'text-red-800'
                  }`}
                >
